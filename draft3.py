...
def show(*args):
    ...
    for i in range(tot_sub):  # Loop through the number of subjects
        ...
        try:
            avg = (float(attained) / float(max_mark)) * 100
            tot_avg += avg
            word, color = 'Percentage : {:.2f}%'.format(avg), 'green'
            if avg > 101 or float(max_mark) < 0:
                word = 'Wrong Values'
                color = 'red'
    ...
    tot_avg_label.grid(row=tot_sub, column=2, columnspan=2)
    avg_button = Button(root, text='Average', width=20, bg='green', fg='black', command=root.bind('<Return>', show))
    avg_button.grid(row=tot_sub, column=0, columnspan=2)
    # Print the values out


sub = Entry(root, width=15, borderwidth=3)
